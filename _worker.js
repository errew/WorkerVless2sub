...
let socks5Data;
let alpn = 'h3';
let 网络备案 = `<a href='https://www.0200000.xyz'>牛ICP备-20250207号</a>`;//写你自己的维护者广告
let 额外ID = '0';
let 加密方式 = 'auto';
let 网站图标， 网站头像， 网站背景;
...
async function 整理测速结果(tls) {
	...
	const csvPromises = addressescsv。map(async (csvUrl) => {
		try {
			...
			if (!response。ok) {
				throw new 错误(`HTTP错误 ${response。status}: ${response。statusText}`);
			}
	...
	async fetch(request， env) {
		...
		try {
			...
			if (!subConverterResponse。ok) {
				throw new 错误(`Error fetching subConverterUrl: ${subConverterResponse。status} ${subConverterResponse。statusText}`);
			}
